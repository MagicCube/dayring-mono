...
ServiceManager starts BLEService after TimeService. The service initializes the SDK's existing NimBLE host, registers GATT, waits up to five seconds for host synchronization, and starts connectable advertising named Dayring. It requests SMP security when a central connects. Advertising stops during the single connection and resumes on disconnection. State snapshots cross from the host task to the owning loop through atomics; callbacks never access applications or controllers.

Stop synchronously stops the NimBLE host before releasing callback/GATT storage, then deinitializes it without deleting bonds. Unexpected host-stop/deinit failures are fatal rather than permitting callback storage to be freed while still in use. Failed startup cleans up its initialized resources. Native tests and previews report Unavailable without accessing the Mac radio. The backend intentionally keeps its host callbacks and GATT storage together (about 200 lines of class definition) because their lifetimes must agree.
...
RPC, application authorization, passkey UI, automatic reconnect and production pairing-window policy are separate follow-up work.
