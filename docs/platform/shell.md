...
LockPage renders a solid black background with centered text: a white uppercase Roboto L date starting 40 px below the top, white Ndot 57 time (`NDot120`), and light-gray Roboto M `CHARGING 85%` text only while charging. Date and time stay in place when the charging line is hidden. LockPageController samples charging and percentage on entry and with each displayed minute change; an unavailable percentage falls back to `CHARGING`. This does not add sleep wake sources or charging-triggered frontlight behavior. Entry uses a single rendered frame without a separate white clearing pass.
