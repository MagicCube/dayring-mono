...
`ServiceManager` is the service owner and startup runner. Its constructor installs FrontlightService, PowerService, TaskDispatchService, then TimeService; add later services in dependency order before the first `begin()`. Pass dependencies through constructors and keep typed borrowed references; no dependency graph or automatic sorting is involved. Future BLEService and Calendar/Weather services follow these built-ins. Every service must live in its owning domain's `services/` directory: hardware services in `hal/services/`, task dispatch in `tasking/services/`, time in `time/services/`, and the lifecycle framework in `runtime/services/`. Future BLE services follow the same rule. Directory organization does not add a `services` namespace; existing domain namespaces remain unchanged.
