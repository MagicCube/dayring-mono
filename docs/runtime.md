...
RPCService borrows BLEService and TaskDispatchService. TimeService borrows RPCService, owns RTC synchronization, and stores the current IANA timezone/UTC offset and sync cadence in memory. Ready-session, eight-hour, and timezone-change triggers are documented in [RPC and time](rpc.md).
