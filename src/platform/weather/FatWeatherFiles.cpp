...
class FatWeatherFiles final : public WeatherFiles {
   public:
    std::optional<std::string> read(unsigned slot) override {
        ...
        if (!file || file.size() > 160) return std::nullopt;
   ...
   private:
    ...
    static const char* _path(unsigned slot) {
        return slot == 0 ? "/weather-a.cache" : "/weather-b.cache";
    }
...
std::unique_ptr<WeatherStorage> makeWeatherStorage() {
    return std::make_unique<FileWeatherStorage>(std::make_unique<FatWeatherFiles>());
}
