...
int main() {
    ...
    tasks.update(30);
    assert(callbacks == 1);
    transport.inject({.id = 99, .method = 1, .payload = {7, 8}});
    tasks.update(40);
    ...
    const auto queued = rpc.request(1, {}, [](auto, auto) {}, 130);
    assert(queued && rpc.cancel(queued));
    ...
    rpc.stop();
    assert(rpc.start());
    transport.inject({.id = 501, .method = helloMethod}, 2);
    tasks.update(30);
    const std::vector<uint8_t> large(maxPayloadSize, 42);
    assert(!rpc.request(42, large, [](auto, auto) {}, 30));
    transport.inject({.id = 502, .method = helloMethod, .payload = {2}}, 2);
    tasks.update(40);
    reply = decode({transport.outbound.back().bytes.data(), transport.outbound.back().size});
    assert(reply && reply->payload == std::vector<uint8_t>{2});
    assert(!rpc.request(42, std::vector<uint8_t>(maxPayloadSize + 1), [](auto, auto) {}, 40));
    assert(rpc.registerHandler(42, [](auto) { return Reply{.payload = std::vector<uint8_t>(maxPayloadSize + 1)}; }));
    transport.inject({.id = 503, .method = 42}, 2);
    tasks.update(50);
    reply = decode({transport.outbound.back().bytes.data(), transport.outbound.back().size});
    assert(reply && reply->kind == Kind::Error && reply->payload[0] == 2);
    const auto bigID = rpc.request(42, large, [&](Error e, auto) { result = e; }, 50);
    assert(bigID);
    transport.generation = 0;
    tasks.update(60);
    assert(result == Error::Disconnected);
