...
def parser() -> Parser:
    ...
    capture = commands.add_parser("capture", help="Capture one exact URL to PNG", description=
                                  ...
                                  "time 12:34, battery 75%, charging off. Repeated inputs overwrite the same PNG.")
    ...
    capture.add_argument("--time", type=clock, default="12:34", help="Fixed device time, HH:MM")
    capture.add_argument("--battery", type=battery, default=75, help="Battery percentage, 0..100")
    capture.add_argument("--charging", action="store_true", help="Show the charging indicator")
    ...
    for command in (capture, routes, help_command):
...
def capture(args: argparse.Namespace, executable: Path, cache_hit: bool) -> dict:
    ...
    metadata, pixels = native(executable, ["capture", args.url, hour, minute, str(args.battery), str(int(args.charging))])
    ...
    state = {"time": args.time, "battery": args.battery, "charging": args.charging}
...
def main() -> int:
    ...
    try:
        ...
        executable, cache_hit = build(rebuild=args.rebuild)
